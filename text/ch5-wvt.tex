\chapter{\label{ch:5-msd}A wavelet transform method to determine monsoon onset and retreat from precipitation time-series}
...
For this reason, a plethora of methods have been used to diagnose the onset and retreat dates from a range of variables and datasets.
...
The goal of this chapter is then to present an objective approach that is more portable across datasets, regions and robust for various purposes, some of which will be further explored in the following chapter.   
...
Table \ref{tab:3} shows the onset and retreat dates of the North American Monsoon estimated using the \citetalias{geil2013}, \citetalias{arias2012} and WT methods.
The table reports the results for three observational datasets, ERA-5 reanalysis and five climate model experiments.
The observations generally agree that the onset date is found at pentad 33 (around June 15), according to the WT and the method by \citetalias{geil2013}.
However, the method of \citetalias{geil2013} generally places the retreat date one pentad after the WT method, i.e., around October 7th for G13's method and October 2nd for the WT's method.
The method by \citetalias{arias2012} disagrees with \citetalias{geil2013} and the WT methods on both onset and retreat mean pentads, in both cases finding an earlier onset (pentad 30) and retreat (pentad 54). 
...
The composite differences of the precipitation, wind and geopotential changes 10 days prior and after onset and retreat are compared for the three methods. The impact of monsoon onset in precipitation is sligthly stronger using the method by \citetalias{arias2012} than the WT or \citetalias{geil2013}. 
The WT method shows a very similar pattern and magnitudes of the anomalies of onset and retreat when compared to the other two methods. The method by \citetalias{geil2013} produces the weakest anomalies, particularly of precipitation, but very similar patterns overall.
...
Due to this year-to-year variability in the characteristics of the seasonal cycle, an objective measure was defined to determine whether a signal presented a robust MSD-bimodal signature.  
...
The evaluation of the MOHC submissions to CMIP6 in Chapter \ref{ch:4-ams} lacked a robust assessment of the timings of the monsoon systems in the Americas. 
